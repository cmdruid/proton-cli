...
`--from` and `--to` match addresses; `--keyword` also matches display names and body text. Proton's index lags a change by a few seconds ([why](../design-notes.md#why-search-lags)).
...
Each JSON line names the thread in `conversation_id`, so a consumer can act on the whole thread without looking it up. See [Streams](../output.md#streams) and [Desktop notifications](../scripting.md#desktop-notifications-mail-and-calendar-reminders).
