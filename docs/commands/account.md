...
proton-cli account logout --revoke     # also invalidate it at Proton
...
$ proton-cli account profiles delete old
✓ Deleted profile "old".
```
...
```bash
proton-cli account sessions revoke 3Ns8pT2v
proton-cli account sessions revoke --others     # everything but this one
```
...
```bash
proton-cli account settings get     # the values now in effect
proton-cli account settings list    # the keys you can change
proton-cli account settings set locale de_AT
```

```console
$ proton-cli account settings list
...
```bash
proton-cli mail settings get
proton-cli calendar settings get
proton-cli drive settings get
```
