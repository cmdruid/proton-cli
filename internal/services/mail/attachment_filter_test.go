...
func TestAttachmentIsInline(t *testing.T) {
	tests := []struct {
		...
		{"", false},
		{"embed", false},
		{"INLINE", false}, // case-sensitive: matches what the API sends
	}
	for _, tc := range tests {
		t.Run(tc.disposition, func(t *testing.T) {
			got := Attachment{Disposition: tc.disposition}.IsInline()
			if got != tc.want {
