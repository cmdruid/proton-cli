...
Examples:
  ...
  proton-cli update --force    # reinstall the latest even if up to date`,
		...
		RunE: run(nil, func(c *Invocation) error {
			...
			return runUpdate(c, target, checkOnly, force)
		}),
	}
...
func runUpdate(c *Invocation, target string, checkOnly, force bool) error {
	r := c.R()
	...
	if checkOnly {
		if r.Format != render.FormatText {
			return r.Object(updateStatus{Current: version, Latest: latest, UpdateAvailable: newer})
		}
		if newer {
			r.Info(fmt.Sprintf("Update available: %s -> %s. Run `proton-cli update` to install it.", version, latest))
		} else {
			r.Info(fmt.Sprintf("proton-cli is up to date (%s).", version))
		}
		return nil
	}
	...
	if !newer && target == "" && !force {
		if r.Format != render.FormatText {
			return r.Object(updateStatus{Current: version, Latest: latest, UpdateAvailable: false})
		}
		r.Info(fmt.Sprintf("proton-cli is already up to date (%s).", version))
		return nil
	}
	...
	r.Info(fmt.Sprintf("Downloading proton-cli %s...", latest))
	bin, err := selfmanage.Download(c.Ctx, client, latest)
	if err != nil {
		return err
	}
	r.Info("Verified checksum. Installing...")
	if err := selfmanage.Apply(bin, exe); err != nil {
		return selfManageError(err, exe, actionUpdate)
	}

	if r.Format != render.FormatText {
		return r.Object(updateStatus{Current: latest, Latest: latest, UpdateAvailable: false})
	}
	r.Success(fmt.Sprintf("Updated proton-cli %s -> %s.", version, latest))
	return nil
}
