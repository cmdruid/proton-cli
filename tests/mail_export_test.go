...
func TestMailSendFromEMLRejectsAnUnreadableFile(t *testing.T) {
	...
	_, stderr, code := run(t, "mail", "messages", "send", "--eml", path)
	if code == 0 {
		t.Error("a file that is not mail should be refused")
	}
	if !strings.Contains(stderr, "eml") && !strings.Contains(stderr, "body") {
		t.Errorf("expected a message naming the file or its problem, got: %s", stderr)
	}
}
